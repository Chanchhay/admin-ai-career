...
export default function CompanyDetailPage() {
  ...
  const submit = async (decision: Decision) => {
    ...
    try {
      ...
      setNote("");
      toast.success(
        tx("Decision recorded: {decision}.", {
          decision: tx(humanizeEnum(decision)),
        }),
      );
    } catch (error) {
      toast.error(
  ...
  if (isError || !data) {
    return (
      <ErrorState
        message={tx("Unable to load this company.")}
        onRetry={refetch}
      />
    );
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <Panel>
        ...
        <dl className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <Field label="Profile status" value={tx(humanizeEnum(company.status))} />
        </dl>
      </Panel>
            ...
            {documents.map((document) => (
              <li key={document.id}>
                ...
                >
                  ...
                  <GhostChip>{tx(humanizeEnum(document.status))}</GhostChip>
                </a>
              </li>
            ))}
          </ul>
      ...
      <StartConversation companyId={id} label={tx("Message recruiter")} />

      <Panel>
        <PanelHeader
          title={tx("History ({count})", {
        ...
        {verificationHistory.length === 0 ? (
          <p className="rounded-[22px] bg-ws-card-hover px-5 py-8 text-center text-sm text-ws-faint">
            {tx("No decision has been recorded yet.")}
          </p>
        ) : (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  <Chip tone={entry.decision === "APPROVED" ? "solid" : entry.decision === "REJECTED" ? "alert" : "soft"}>
                    {tx(humanizeEnum(entry.decision))}
                  </Chip>
                  <span className="text-xs text-ws-faint">
                    {tx("{date} · moderator #{id}", {
                      date: formatDateTime(entry.verifiedAt),
                      id: entry.moderatorProfileId,
                    })}
                  </span>
                </div>
                {entry.note ? (
