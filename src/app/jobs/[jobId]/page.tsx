...
export default function JobDetailPage({ params }: PageProps<"/jobs/[jobId]">) {
  ...
  useSetPageHeading(job?.title ?? tx("Job details"));

  if (!Number.isFinite(id)) return <ErrorState message={tx("This job ID is invalid.")} />;
  if (isLoading) return <Panel><LoadingState rows={7} /></Panel>;
  if (isError || !job) return <ErrorState message={tx("Unable to load this published job.")} onRetry={refetch} />;
  ...
  return (
    <div className="flex flex-col gap-5">
      <Link href="/jobs" className="inline-flex w-fit items-center gap-2 text-sm font-semibold text-ws-muted hover:text-ws-fg">
        <ArrowLeft aria-hidden="true" className="size-4" /> {tx("Back to jobs")}
      </Link>
      <Panel>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {job.experienceLevel ? <GhostChip>{tx(humanizeEnum(job.experienceLevel))}</GhostChip> : null}
            </div>
          </div>
        </div>
      </Panel>
        ...
        <div className="flex flex-col gap-5">
          <Panel>
            ...
            <dl className="space-y-3 text-sm">
              <Detail label={tx("Salary")} value={formatSalary(job.salaryMin, job.salaryMax, tx)} />
              <Detail label={tx("Published")} value={formatDate(job.publishedAt)} />
              <Detail label={tx("Expires")} value={formatDate(job.expiredAt)} />
            </dl>
          </Panel>
          <Panel>
            <PanelHeader title="Skills" />
            {job.skills?.length ? <div className="flex flex-wrap gap-2">{job.skills.map((skill) => <GhostChip key={skill.id}>{skill.skillName}{skill.requiredLevel ? ` · ${tx(humanizeEnum(skill.requiredLevel))}` : ""}</GhostChip>)}</div> : <p className="text-sm text-ws-faint">{tx("No skills listed.")}</p>}
          </Panel>
        </div>
      </div>
    </div>
...
function formatSalary(
  ...
  if (min == null && max == null) return tx("Not disclosed");
  const format = (value: number) => new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(value);
  if (min != null && max != null) return tx("{min} – {max}", { min: format(min), max: format(max) });
  return min != null ? tx("From {min}", { min: format(min) }) : tx("Up to {max}", { max: format(max!) });
}
