...
import { GatewayNotice } from "@/components/layout/GatewayNotice";
import {
  PageHeadingProvider,
  usePageHeading,
...
function Frame({ children }: { children: ReactNode }) {
  ...
  // Checked before anything else renders: if the gateway can't be reached,
  // every query below 401s the same way, so there is nothing this shell can
  // usefully show instead.
  const { data: session, isSuccess: sessionChecked } = useGetSessionQuery();
  if (sessionChecked && session.reachable === false) {
